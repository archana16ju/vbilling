...
const uri = 'mongodb+srv://vseyal_team:hTdWg5Rp6XEePqdu@cluster0.pcmcfdv.mongodb.net/skbriyani?appName=Cluster0';
...
async function run() {
  try {
    ...
    // Insert new
    await db.collection('products').insertMany(PRODUCTS.map(p => ({ ...p, _id: p.id })));
    await db.collection('categories').insertMany(CATEGORIES.map(c => ({ ...c, _id: c.id })));
    
    console.log("Successfully inserted products and categories into the remote MongoDB!");
  } finally {
    await client.close();
  }
}
